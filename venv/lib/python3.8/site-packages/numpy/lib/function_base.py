...
class vectorize:
    ...
    >>> vfunc.__doc__
    'Return a-b if a>b, otherwise return a+b'
    >>> vfunc = np._vectorize(myfunc, doc='Vectorized `myfunc`')
    >>> vfunc.__doc__
    'Vectorized `myfunc`'
    ...
    >>> type(out[0])
    <class 'numpy.int64'>
    >>> vfunc = np._vectorize(myfunc, otypes=[float])
    >>> out = vfunc([1, 2, 3, 4], 2)
    >>> type(out[0])
    <class 'numpy.float64'>
    ...
    ...         res = res*x + _p.pop(0)
    ...     return res
    >>> vpolyval = np._vectorize(mypolyval, excluded=['p'])
    ...
    >>> import scipy.stats
    >>> pearsonr = np._vectorize(scipy.stats.pearsonr,
    ...                 signature='(n),(n)->(),()')
    ...
    >>> convolve = np._vectorize(np.convolve, signature='(n),(m)->(k)')
